...
function draw(selected_year){
  ...
  d3.csv("data/new_locations_continents.csv", function(data){
    function display_hdi(d = d => d.Country_Name == 'United States'){
      console.log(d)
      document.getElementById("demo").innerHTML = "<img src=\"./SVG_Images/"+d.Year + ".png\"   >" + "<br>"+
    ...
    var dateTime2 = [1896, 1900, 1904, 1908, 1912, 1920, 1924, 1928, 1932, 1936, 1948, 1952, 1956, 1960, 1964, 1968, 1972, 1976, 1980, 1984, 1988, 1992, 1996, 2000, 2004, 2008, 2012, 2016, 2020];

    var dataTime = dateTime2.map(function(d) {
    ...
    var x = d3.scaleLinear().domain( [1890, 2021]).range([ 0, 400]);

    svg2.append("g").attr("transform", "translate(0," + 300 + ")").call(d3.axisBottom(x));

    // Add Y axis
    var y = d3.scaleLinear().domain( [0, 300]).range([ 300, 0 ]);
  
    var y1 = d3.scaleLinear().domain( [0, 1]).range([ 300, 0 ]);

    svg2.append("g").style("fill", "red").call(d3.axisRight(y));
      
    svg2.append("g").call(d3.axisLeft(y1)).style("fill", "green").attr("transform", "translate(400," + 0 + ")");
